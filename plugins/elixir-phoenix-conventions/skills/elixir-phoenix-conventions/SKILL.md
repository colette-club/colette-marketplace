...
- A second `Repo`, schema, or query pointed at another application's database/tables, or a context branching on which client is calling (`if client == "mobile"`) → cross the boundary through a published contract instead — the API, an event, or an adapter behind a behaviour (#5, `core #12`).
