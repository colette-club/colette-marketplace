...
23. Single level of abstraction; small functions; extract `defp`s (highest-risk #5). **After a change — especially removing logic — re-read the code it touched and clean up what the edit made pointless in the SAME change:** a helper collapsed to `x -> x` gets inlined at its call site and deleted; a now-unreachable clause, an `@attr` left unused, or a comment that no longer matches reality gets removed. Don't leave dead scaffolding behind.
...
- A just-edited change left a now-trivial wrapper (`defp f(x), do: x`), an unreachable clause, or an unused `@attr`/stale comment behind → inline/remove it in the same change (after editing, re-check what your edit made pointless).
