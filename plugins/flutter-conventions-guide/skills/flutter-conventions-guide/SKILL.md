...
53. **App guides** (`GuideRegistry` + `guidi_client`): follow the GuideService timer pattern verbatim (cancellable Timer, `_guideIsPlaying` lock, `isCurrent` guard at every async boundary, reschedule on not-current, cancel in dispose). Every guide MUST reach a terminal beat that calls `markGuideSeen` — the player's `onGroupFinished` does **not** fire when the last shown step is `tapThrough`, so a guide ending on (or whose custom interactive flow continues past) a `tapThrough` step must call `markGuideSeen` itself in its finish handler, or the guide is never marked seen and **replays on every visit**. Each step's `targetKey` must be attached to a live widget (`key: GuideRegistry.x`); a never-mounted target is silently skipped (nothing highlights). When you remove or rename guided UI, delete the matching key + step (and any bespoke playback plumbing) so no orphan targets or dead resume paths linger. Audience-gate intros with `GuideAudience` (`newUser`/`returningUser`/`all`) — but `unseenForScreen` does **not** filter by audience, so gate at the auth transitions: on onboarding call `markGuideNotApplicable` for every `returningUser` guide (new users skip existing-user intros), and on returning-user login do the same for every `newUser` guide. A feature shipped *after* some members onboarded needs a `returningUser` guide to reach them — they already completed the `newUser` welcome tour and won't replay it.
